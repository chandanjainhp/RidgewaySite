...
const SEED_LOCATIONS = {
  gate3: { id: "loc:gate_3", lat: 51.505, lng: -0.1005, name: "Gate 3" },
  southGate: {
    id: "loc:south_gate",
    lat: 51.495,
    lng: -0.1005,
  ...
  accessPoint7: {
    id: "loc:access_point_7",
    lat: 51.498,
    lng: -0.099,
    name: "Access Point 7",
  },
  blockC: { id: "loc:block_c", lat: 51.5025, lng: -0.098, name: "Block C" },
  storageYardB: {
    id: "loc:storage_yard_b",
    lat: 51.5,
    lng: -0.102,
    name: "Storage Yard B",
  },
  road4: {
    id: "loc:road_4_checkpoint",
    lat: 51.5,
    lng: -0.095,
...
const seedEvents = [
  // 01:10 — Badge swipe failures (employee at wrong gate)
  {
    ...
    nightDate: getLocalYesterdayDateString(),
    type: "badge_swipe_fail",
    severity: "harmless",
    ...
    rawData: {
      ...
      observations: [
        ...
        { location: "Block C", finding: "untagged_vehicle near loading bay" },
        {
          ...
          finding: "authorized contractor vehicle V-09 unlogged",
        },
        {
          location: "Access Point 7",
          finding: "multiple badge failures, no tailgating observed",
        },
      ],
      unresolvedVehicle: "V-UNKNOWN-01",
    },
    description:
      "Drone patrol observation - untagged vehicle near Block C loading bay",
  },
...
const seedEmployees = [
  {
    ...
    notes:
      "Usually accesses via East Gate; West Gate under maintenance tonight",
  },
];
...
const seedDronePatrols = [
  {
    ...
    waypoints: [
      ...
      },
      {
        ...
        observation:
          "Three badge failures reviewed, employee likely at wrong gate",
      },
    ],
    status: "complete",
  },
...
const seedSiteNotes = [
  {
    ...
    content:
      "Please check Block C before leadership asks. Drone spotted untagged vehicle near loading bay at 03:12. Need clarification on V-09 arrival — not in advance log.",
    priority: "high",
...
const seedDataExists = async () => {
  try {
    const nightDate = getLocalYesterdayDateString();
...
export const seedOvernightData = async () => {
  try {
    ...
    if (dataExists) {
      console.log(
        `[Seed] ✓ Data already exists for ${nightDate} — skipping seed`,
      );
      return;
    }
    ...
    console.log(
      `[Seed] ✓ Inserted ${patrolResult.insertedCount} drone patrols`,
    );
    ...
    console.log(`[Seed] ✓ nightDate used: ${nightDate}`);
...
export const seedIncidents = async () => {
  try {
    ...
    const evt004 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-004",
    });
    const evt001 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-001",
    });
    const evt005 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-005",
    });
    const evt006 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-006",
    });
    const evt007 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-007",
    });
    const evt002 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-002",
    });
    const evt003 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-003",
    });
    const evt008 = await eventsCollection.findOne({
      eventId: "EVT-2026-04-15-008",
    });
    ...
    const seedIncidentsData = [
      {
        ...
        description:
          "Drone observed untagged vehicle near Block C loading bay; requires leadership-ready explanation",
        eventIds:
          evt004 && evt005 && evt006 && evt007
            ? [evt004._id, evt005._id, evt006._id, evt007._id]
            : [],
        location: {
          name: "Block C",
          coordinates: {
      ...
      },
      {
        ...
        description:
          "Fence sensor alert at Gate 3 likely caused by loose wire; drone confirmed harmless",
      ...
      },
      {
        ...
        description:
          "Three badge failures at Access Point 7 between 01:10 and 01:13 by employee at wrong gate",
      ...
      },
      {
        ...
        description:
          "Canteen motion sensor triggered by fox and admin lighting timer malfunction",
    ...
    const incidentResult =
      await incidentsCollection.insertMany(seedIncidentsData);
...
export const clearSeedData = async () => {
  try {
    ...
    console.log(
      `[Seed] ✓ Deleted ${vehicleDeleteResult.deletedCount} vehicles`,
    );

    const employeesCollection = getCollection("employees");
    const employeeDeleteResult = await employeesCollection.deleteMany({});
    console.log(
      `[Seed] ✓ Deleted ${employeeDeleteResult.deletedCount} employees`,
    );

    const patrolsCollection = getCollection("dronePatrols");
    const patrolDeleteResult = await patrolsCollection.deleteMany({});
    console.log(
      `[Seed] ✓ Deleted ${patrolDeleteResult.deletedCount} drone patrols`,
    );

    const notesCollection = getCollection("siteNotes");
    const notesDeleteResult = await notesCollection.deleteMany({});
    console.log(
      `[Seed] ✓ Deleted ${notesDeleteResult.deletedCount} site notes`,
    );
    ...
    console.log(
      `[Seed] ✓ Deleted ${incidentDeleteResult.deletedCount} incidents`,
    );
