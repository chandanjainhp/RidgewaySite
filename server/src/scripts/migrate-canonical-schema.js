...
async function migrateIncidents() {
  ...
  await batchUpdate(
    ...
    (doc) => ({
      severity:
        SEVERITY_MAP[doc.finalSeverity] || doc.finalSeverity || "uncertain",
    }),
  ...
  if (APPLY) {
    await col.updateMany(
      { primaryLocation: { $exists: true } },
      { $unset: { primaryLocation: "" } },
    );
  ...
  await batchUpdate(
    ...
    (doc) => ({
      correlation: { ...(doc.correlation || {}), type: doc.correlationType },
    }),
    "correlationType→correlation.type",
  );
  if (APPLY) {
  ...
  if (isOldShape || !APPLY) {
    ...
    if (APPLY && count > 0) {
      ...
      for (const doc of docs) {
        ...
        const sections = OLD_SECTION_KEYS.filter((k) => k in doc.sections).map(
          (k) => {
            const sec = doc.sections[k];
            const content = sec?.mayaVersion || sec?.agentDraft || "";
